...
"Estás asistiendo al técnico de préstamos en la interfaz de 'Materiales'. " +
"Aquí puedes agregar materiales y asignarlos a los laboratorios correspondientes. Para registrar un material, completa los siguientes campos: " +
"código del material, laboratorio al que pertenece (usa el botón 'Ver laboratorios' para seleccionarlo), tipo de equipo, número de serie y la cantidad de unidades que estás añadiendo. " +
"Una vez llenado todo, presiona el botón 'Agregar' para guardarlo. " +
"También puedes modificar los datos de un material, habilitarlo o deshabilitarlo, y eliminarlo si es necesario. " +
"Si necesitas ayuda en algún paso, solo dime y te ayudo encantado 😊. "
        );
...
private void cargarNombreCompleto() {
    ...
    private void cargarTablaPorCategoria() {
        ...
        try (Connection con = Conexion.obtenerConexion()) {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT m.id_material, m.nombre, m.tipo_equipo, m.numero_serie, m.estado, l.Codigo_lab, m.unidades, l.seccion "
                    ...
                    + "WHERE l.seccion = ? AND m.estado_material = 1 ");
    ...
    "FROM materiales m " +
    "JOIN laboratorios l ON m.ID_lab = l.ID_lab " +
    "WHERE l.Seccion <> 'Hardware' AND m.estado_material = 1 "
);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Object[] fila = {
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        ...
        int confirm = JOptionPane.showConfirmDialog(null, "¿Estás seguro de deshabilitar este material?", "Confirmar", JOptionPane.YES_NO_OPTION);

        if (confirm == JOptionPane.YES_OPTION) {
            try (Connection con = Conexion.obtenerConexion()) {
                // Actualizar estado_material a 0 (borrado lógico)
                PreparedStatement ps = con.prepareStatement("UPDATE materiales SET estado_material = 0 WHERE id_material = ?");
                ps.setInt(1, idMaterial);
                ps.executeUpdate();

                JOptionPane.showMessageDialog(null, "Material deshabilitado correctamente.");
                ...
                String detalleLog = "Usuario: '" + usuarioTecnico + "' Rol: '" + rolTecnico + "' deshabilitó el material: nombre '" + codigoText
                        + "', tipo '" + tipoEquipoText + "', número de serie '" + NumSerieText
                        + "', unidades " + unidadesText + ", laboratorio '" + laboratorioText + "'.";

                LogManager.registrarLog(idTecnico, rolTecnico, accion, detalleLog);

                cargarTablaTodo(); // asegúrate que esta consulta solo muestre materiales con estado_material = 1
                limpiar();
            } catch (SQLException e) {
