...
public class VentanaGraficosMultiplesPrestamos extends JFrame {
    private DefaultTableModel model;

    public VentanaGraficosMultiplesPrestamos(DefaultTableModel model) {
        this.model = model;
        setTitle("Gráficos de Préstamos");
        setDefaultCloseOperation(DISPOSE_ON_CLOSE);
        setSize(1000, 800);
        setLocationRelativeTo(null);
        setLayout(new GridLayout(2, 2)); // Mostrar 4 gráficos en una cuadrícula 2x2
    ...
    private JPanel crearPanelBarraPorEstado() {
        ...
        for (int i = 0; i < model.getRowCount(); i++) {
            String estado = model.getValueAt(i, 7).toString(); // Índice 7 para estado
